...
class GatedSpectralClassification(nn.Module):
    def __init__(self, hidden_dim, num_classes, alpha=0.5):
        super().__init__()
        
        self.hidden_dim = hidden_dim
        self.num_classes = num_classes
        self.alpha = alpha  # Graph fusion weight (α in paper)
        
        # Entropy-weighted gating
        self.entropy_gate = EntropyWeightedGating(hidden_dim)
        
        # Spectral GNN
        self.spectral_gnn = SpectralGNN(hidden_dim, num_classes, alpha=alpha)
        
    def forward(self, features, speaker_graph, temporal_graph, qmask, return_gates=False):
        """
        ...
        Args:
            ...
            return_gates: If True, also return gating weights for regularization
        Returns:
            output: Classification logits
            gates: Gating weights (if return_gates=True)
        """
        ...
        fused_features, gates = self.entropy_gate(
            features, 
...
class EntropyWeightedGating(nn.Module):
    ...
    def forward(self, features, speaker_graph, temporal_graph, entropy_s, entropy_t, qmask):
        ...
        fused = {}
        all_gates = []  # Collect all gates for regularization
        ...
        # Stack gates for regularization: Z^k shape [batch, seq_len, hidden_dim]
        gates_tensor = torch.stack(all_gates, dim=2)  # [batch, seq_len, n_modalities, hidden_dim]
            
        return fused, gates_tensor

class SpectralGNN(nn.Module):
    def __init__(self, hidden_dim, num_classes, num_layers=3, alpha=0.5):
        super().__init__()
        
        self.alpha = alpha  # Graph fusion weight (α in paper Equation 7)
    ...
    def forward(self, features, speaker_graph, temporal_graph):
        ...
        # Fuse adjacency matrices: A = α * E^s + (1-α) * E^q (Paper Equation 7)
        fused_adj = self.alpha * speaker_graph + (1 - self.alpha) * temporal_graph
