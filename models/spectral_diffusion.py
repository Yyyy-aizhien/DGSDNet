...
class DualSpectralDiffusion(nn.Module):
    ...
    def forward(self, features, speaker_graph, temporal_graph, missing_mask=None, return_loss=False):
        """
        ...
        Args:
            ...
            return_loss: If True, also return reconstruction loss
        Returns:
            recovered: Reconstructed features
            recon_loss: Reconstruction loss (if return_loss=True)
        """
        ...
        for modality in ['text', 'audio', 'visual']:
            ...
            if missing_mask is not None and missing_mask[:, :, modality_idx].any():
                ...
                # Calculate reconstruction loss for missing modalities
                if return_loss:
                    # MSE between reconstructed and original (for supervision)
                    missing_indices = missing_mask[:, :, modality_idx]
                    if missing_indices.any():
                        recon_loss += torch.mean((recovered[modality][missing_indices] - features[modality][missing_indices]) ** 2)
            else:
                recovered[modality] = features[modality]
        
        if return_loss:
            return recovered, recon_loss
        return recovered
