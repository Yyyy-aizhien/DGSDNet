...
def train_missing_rate_experiment(dataset_name='iemocap', missing_rate=0.0, cv_fold=0, epochs=50, 
                                  ablation_type='full', hidden_dim=32):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_pbar):
            try:
                ...
                # Update progress bar
                train_pbar.set_postfix({'loss': f'{loss.item():.4f}', 'grad': f'{grad_norm:.3f}'})
...
def run_missing_rate_experiments(hidden_dim=32):
    ...
    # Save results
    results_file = 'missing_rate_results.json'
...
if __name__ == "__main__":
    ...
    if args.run_all:
        run_missing_rate_experiments(hidden_dim=args.hidden_dim)
    else:
        waf1 = train_missing_rate_experiment(
